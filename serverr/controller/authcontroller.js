...
const login = async (req, res, next) => {
  try {
    ...
    if (!email || !password) {
      return res
        .status(400)
        .json({ message: "Please provide the email or pssword" });
    }
    ...
    if (!user || !(await bcrypt.compare(password, user.password))) {
      return res
        .status(400)
        .json({ message: "Please provide the  correct email or pssword" });
    }
...
const protect = async (req, res, next) => {
  try {
    ...
    const ispasschange = await user.passwordchanged(decoded.iat);
    if (ispasschange) {
      return res.status(401).json({
        success: false,
  ...
  return (req, res, next) => {
    try {
      ...
      return res.status(500).json({
        success: false,
        message: "Server error in restrict middleware",
      });
    }
  };
};
const forgetpassword = async (req, res, next) => {
  try {
    ...
    const reseturl = `http://localhost:3001/reset/${encodeURIComponent(
      resettoken
    )}`;
    ...
    try {
      ...
      return res.status(500).json({
        ...
        error: error.stack,
        message:
          "There was an error in the sending reset password.Please try again later",
      });
    }
  } catch (error) {
    return res.status(401).json({
